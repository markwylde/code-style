...
function loadPages(): Page[] {
  return Object.entries(markdownModules)
    .map(([path, raw]) => {
      ...
      const id = typeof meta.id === 'string' && meta.id ? meta.id : deriveIdFromPath(path)
      const title = typeof meta.title === 'string' && meta.title ? meta.title : deriveTitleFromPath(path)
...
function parseHash(hash: string, pages: Page[]): ViewState {
  ...
  return { section: 'guide', guideId: resolveGuideId(firstPart || undefined, pages) }
}
...
function splitIntroFromContent(html: string): { introHtml: string; contentHtml: string } {
  const container = document.createElement('div')
  container.innerHTML = html

  const introNodes: Element[] = []
  while (container.firstElementChild && container.firstElementChild.tagName === 'P') {
...
export default function App(): JSX.Element {
  ...
  const [view, setView] = useState<ViewState>(() => parseHash(window.location.hash, pages))
  const [isGuideNavOpen, setGuideNavOpen] = useState(false)
  const [specMode, setSpecMode] = useState<SpecMode>('friendly')
  const [specCopyState, setSpecCopyState] = useState<'idle' | 'copied' | 'error'>('idle')
  ...
  useEffect(() => {
    const handleHashChange = () => {
      setView((previous) => {
        ...
        if (nextView.section === previous.section && nextView.guideId === previous.guideId) {
          return previous
        }
        return nextView
      })
  ...
  useEffect(() => {
    const targetHash = createHash({ section: view.section, guideId: resolveGuideId(view.guideId, pages) })
  ...
  const mainClassName = view.section === 'guide' ? 'layout-guide' : 'layout-single'
      ...
      <main className={mainClassName}>
        {view.section === 'guide' && activePage ? (
          <>
            ...
            <article id={guideArticleId} className="page-container">
              {activePage.tagline ? <div className="page-tagline">{activePage.tagline}</div> : null}
              ...
              {pageHtml.introHtml ? (
                ...
                <div className="page-intro" dangerouslySetInnerHTML={{ __html: pageHtml.introHtml }} />
              ) : null}
              {/* biome-ignore lint/security/noDangerouslySetInnerHtml: HTML is generated from trusted markdown */}
              <div className="page-content" dangerouslySetInnerHTML={{ __html: pageHtml.contentHtml }} />
            </article>
          </>
        ) : null}
        ...
        {view.section === 'spec' ? (
          <article id={specArticleId} className="static-page spec-page page-container">
            <div className="spec-toolbar" role="toolbar" aria-label="Spec view options">
              ...
              <button type="button" className={specCopyClassName} onClick={() => { void handleCopySpecMarkdown() }}>
                {specCopyState === 'copied' ? 'Copied!' : 'Copy Markdown'}
              </button>
            </div>
            {specMode === 'friendly' ? (
              /* biome-ignore lint/security/noDangerouslySetInnerHtml: HTML is generated from trusted markdown */
              <div className="spec-friendly" dangerouslySetInnerHTML={{ __html: specHtml }} />
            ) : (
              <pre className="spec-raw">{specMarkdown}</pre>
            )}
            {specCopyState === 'error' ? <p className="spec-copy-error">Unable to copy. Try again.</p> : null}
          </article>
        ) : null}
      </main>
      {view.section === 'guide' ? (
