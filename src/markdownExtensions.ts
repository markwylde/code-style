  ...
  extensions: [
    {
      ...
      start(src) {
        const match = src.match(
          /^:::(callout|info|warning|danger|success|tip|caution)/m,
        );
      ...
      renderer(token) {
        ...
        const titleHtml = calloutToken.title
          ? `<strong>${escapeHtml(calloutToken.title)}</strong>`
          : "";
        const bodyHtml = (
          marked.parse(calloutToken.body, { async: false }) as string
        ).trim();
        const transformedBody = transformCalloutBody(bodyHtml);
        const content = [titleHtml, transformedBody].filter(Boolean).join("\n");
        const className =
          calloutToken.variant === "callout"
            ? "callout"
            : `callout callout-${calloutToken.variant}`;
    ...
    },
    {
      ...
      renderer(token) {
        ...
        if (items.length === 0) {
          return (
            marked.parse(accordionToken.body, { async: false }) as string
          ).trim();
        }
...
function parseAccordionItems(body: string): AccordionItem[] {
  ...
  while (match !== null) {
    ...
    const answerHtml = answer
      ? (marked.parse(answer, { async: false }) as string).trim()
      : "";
...
function renderKeyIdeas(listElement: HTMLElement): string {
  ...
  const ideaHtml = items
    .map((item) => renderKeyIdea(item))
    .filter(Boolean)
    .join("\n");
  if (!ideaHtml) {
    return "";
