...
- Build `createServer(context)` that wires routes with string patterns and adds a `/health` readiness endpoint.
...
Each section includes copy‑pasteable snippets and concrete conventions (e.g., `controllers/users/[id]/get.ts`, string pattern routing, zod schemas, Drizzle models). Start small, keep functions pure, and let the context do the wiring.
